import { unref } from "vue"
import { createLoading } from "@/components/loading/src/createLoading"
import type { LoadingProps } from "@/components/loading/src/props"
import type { Ref } from "vue"

export interface UseLoadingOptions {
  target?: any
  props?: Partial<LoadingProps>
}

interface Fn {
  (): void
}

export type LoadingReturnType = {
  open: Fn
  close: Fn
  setText: (text: string) => void
}

export function useLoading(props?: Partial<LoadingProps>): LoadingReturnType
export function useLoading(opt?: Partial<UseLoadingOptions>): LoadingReturnType
export function useLoading(
  opt: Partial<LoadingProps> | Partial<UseLoadingOptions> = {},
): LoadingReturnType {
  let props: Partial<LoadingProps>
  let target: HTMLElement | Ref<ElRef> = document.body

  if (Reflect.has(opt, "target") || Reflect.has(opt, "props")) {
    const options = opt as Partial<UseLoadingOptions>
    props = options.props || {}
    target = options.target || document.body
  } else {
    props = opt as Partial<LoadingProps>
  }

  const instance = createLoading(props, undefined)

  const open = (): void => {
    const t = unref(target as Ref<ElRef>)
    if (!t) return
    instance.open(t)
  }

  const close = (): void => {
    instance.close()
  }

  const setText = (tip: string) => {
    instance.setText(tip)
  }

  return { open, close, setText }
}
